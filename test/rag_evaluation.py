...
class RAGEvaluator:
    ...
    def evaluate_metrics(self, 
                        ...
                        context_without_pipeline: List[str]) -> Dict[str, Any]:
        """Calculate all metrics for both pipeline and non-pipeline responses"""
        ...
        try:
            ...
            metrics['context_precision_without_pipeline'] = self.context_precision.single_turn_score(without_pipeline_sample)
            
            logger.info("Calculating Context Recall (with pipeline)...")
            metrics['context_recall_with_pipeline'] = self.context_recall.single_turn_score(with_pipeline_sample)
            
            logger.info("Calculating Context Recall (without pipeline)...")
            metrics['context_recall_without_pipeline'] = self.context_recall.single_turn_score(without_pipeline_sample)
            
            logger.info("Calculating Answer Correctness/Faithfulness (with pipeline)...")
            metrics['answer_correctness_with_pipeline'] = self.faithfulness.single_turn_score(with_pipeline_sample)
            
            logger.info("Calculating Answer Correctness/Faithfulness (without pipeline)...")
            metrics['answer_correctness_without_pipeline'] = self.faithfulness.single_turn_score(without_pipeline_sample)
            
            logger.info("Calculating Noise Sensitivity (with pipeline)...")
            metrics['noise_sensitivity_with_pipeline'] = self.noise_sensitivity.single_turn_score(with_pipeline_sample)
    ...
    def write_results_csv(self, results: List[Dict[str, Any]], output_path: str):
        ...
        try:
            ...
            fieldnames = [
                ...
                'answer_correctness_with_pipeline',
                'answer_correctness_without_pipeline',
    ...
    def run_evaluation(self, input_csv_path: str, output_csv_path: str, limit: int = 100):
        ...
        for index, test_case in enumerate(test_cases):
            ...
            try:
                ...
                if not all([product_id, question, ground_truth]):
                    logger.warning(f"Skipping test case {index + 1}: Missing required fields")
                    results.append({
                ...
                if not all([with_pipeline_response, without_pipeline_response, 
                           context_with_pipeline, context_without_pipeline]):
                ...
                result_entry = {
                    ...
                    'question': question,
                    'ground_truth': ground_truth,
                    'context_precision_with_pipeline': metrics.get('context_precision_with_pipeline'),
                    'context_precision_without_pipeline': metrics.get('context_precision_without_pipeline'),
                    'context_recall_with_pipeline': metrics.get('context_recall_with_pipeline'),
                    'context_recall_without_pipeline': metrics.get('context_recall_without_pipeline'),
                    'answer_correctness_with_pipeline': metrics.get('answer_correctness_with_pipeline'),
                    'answer_correctness_without_pipeline': metrics.get('answer_correctness_without_pipeline'),
                    'noise_sensitivity_with_pipeline': metrics.get('noise_sensitivity_with_pipeline'),
                    'status': 'SUCCESS',
                    'error_message': ''
                }
                ...
                logger.info(f"  Context Precision (with): {metrics.get('context_precision_with_pipeline'):.4f}")
                logger.info(f"  Context Precision (without): {metrics.get('context_precision_without_pipeline'):.4f}")
                logger.info(f"  Context Recall (with): {metrics.get('context_recall_with_pipeline'):.4f}")
                logger.info(f"  Context Recall (without): {metrics.get('context_recall_without_pipeline'):.4f}")
                logger.info(f"  Answer Correctness (with): {metrics.get('answer_correctness_with_pipeline'):.4f}")
                logger.info(f"  Answer Correctness (without): {metrics.get('answer_correctness_without_pipeline'):.4f}")
                logger.info(f"  Noise Sensitivity (with): {metrics.get('noise_sensitivity_with_pipeline'):.4f}")
