  ...
  try {
    ...
    if(BASE_MODEL==0){
        ...
        for (let i = 0; i < hypotheticalAnswers.length; i++) {
          ...
          // retrieve top posts for this hypothetical answer
          const hits = await retrieveTopPostsForEmbedding(hypEmb, productName, TOP_K_SEARCH);

          // For each hit ensure we have an embedding (use returned vector if present; else compute)
          // Do embedding computations in parallel but cap concurrency implicitly via Promise.all
          const candidates: Candidate[] = await Promise.all(hits.map(async (h) => {
        ...
        // 6) Apply Reciprocal Rank Fusion (RRF) across the three lists
        const fused = reciprocalRankFusion(mmrReducedLists);
        const finalChunks = fused.slice(0, Math.min(FUSED_TOP, fused.length));

        // 7) Build prompt for final LLM answer (no sentence window retrieval)
        ...
        // Build prompt for final LLM answer (no sentence window retrieval)
        const sources: string[] = [];
        let prompt = `Product: ${productName}\nProduct description: ${productDescription}\n\nContext (posts about this product):\n\n`;
        hits.forEach((c, idx) => {
          const title = c.payload?.title || "";
